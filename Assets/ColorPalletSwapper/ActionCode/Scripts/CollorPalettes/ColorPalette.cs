...
namespace ActionCode.ColorPalettes
{
    ...
    public sealed class ColorPalette : ScriptableObject
    {
        ...
        public Point2[][] GetColorPositionsMatrix()
        {
            ...
            string[] lines = File.ReadAllLines(_colorsPositionFilePath);
            if (lines.Length != colors.Length)
            {
                Debug.LogError("Erro on Colors Position File. Lines number does not match with pallet color number.");
                return null;
            }
            ...
            for (int i = 0; i < matrix.Length; i++)
            {
                ...
                matrix[i] = new Point2[tuplas.Length];
